...
| 字段 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `fallbackChain` | 列表 | `[]` | 按顺序尝试的 `{provider, model}` 配对列表 |
| `fallbackOn` | 列表 | `["RATE_LIMIT","SERVER","TIMEOUT","TRANSPORT","EMPTY_RESPONSE"]` | 触发降级的错误码 |
| `maxFailuresPerModel` | 数字 | `1` | 每个模型连续失败多少次后切换 |
